...
def send_stats_email(filename, summary_df, detail_df):
    try:
        ...
        msg["Subject"] = f"【分單位發送】{filename.replace('.xlsx', '')}"
...
def run_app():
    ...
    if uploaded_files:
        ...
        target_unit = st.sidebar.selectbox("請選擇要設定的單位", sorted(list(units_found)))
        ...
        u_am = st.sidebar.text_input(f"上午尖峰索引 ({target_unit})", value="2, 3", key=f"am_{target_unit}")
        u_pm = st.sidebar.text_input(f"下午尖峰索引 ({target_unit})", value="12, 13", key=f"pm_{target_unit}")
        ...
        if processed_records:
            ...
            st.subheader(f"📝 {target_unit} - 人員明細核對")
            st.info("💡 調整左側規則後，明細會自動重新抓取。您可以點選列後按 Delete 刪除多出的人。")
            ...
            if not edited_df.empty:
                ...
                st.divider()
                st.subheader(f"📊 {target_unit} - 彙整結果")
                ...
                col1.download_button(f"📥 下載 {target_unit} 報表", output.getvalue(), fname, use_container_width=True)
                if col2.button(f"📧 寄送 {target_unit} 結果至信箱", use_container_width=True):
                    with st.spinner("寄送中..."):
                        ok, err = send_stats_email(fname, summary, edited_df)
                        if ok: st.success("✅ 郵件發送成功！")
                        else: st.error(f"❌ 郵件失敗: {err}")
        else:
            st.warning(f"⚠️ 單位『{target_unit}』在目前設定的規則下找不到任何資料。")
