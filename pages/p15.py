...
DEFAULT_CHECKPOINT = pd.DataFrame([
    ...
    {"單位": "三和所", "無線電代號": "", "職別": "警員", "姓名": "謝伯昇", "任務分工": "大門警戒", "臨檢目標場所": "A. 鉅大撞球館、B. 台灣麻將協會、F. 憤怒鳥網咖 Anderson、G. 真情男女養生館、H. 萬紫千紅舒壓館"},
...
# 【智慧算法深度重構】徹底洗掉前端殘留快取、強制連動刷新，確保呼號在排列後「立馬同步修正」
def assign_cp_groups(df):
    ...
    def get_sort_score(row_data, current_idx):
        ...
        if u_text in ["中興所", "龍潭所"] or (u_text == "偵查隊" and current_idx < 5):
            return 11
        return 21
    ...
    # 第二次深度鎖定：【核心關鍵】強制重寫整組代號，徹底洗掉前端 `+ Add row` 自帶的空白殘留，與首列幹部呼號咬死聯動
    for g_name in res['編組'].unique():
        sub_idx = res[res['編組'] == g_name].index
        if len(sub_idx) > 0:
            first_code = res.loc[sub_idx[0], '無線電代號']
            # 不論如何，直接讓整組同仁重新向第一列看齊
            res.loc[sub_idx, '無線電代號'] = first_code
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    for g_name in pdf_ptl_df['編組'].unique():
        ...
        if not sub.empty:
            first_code = sub.iloc[0]['無線電代號']
            pdf_ptl_df.loc[pdf_ptl_df['編組'] == g_name, '無線電代號'] = first_code
    ...
    if df_cp is not None and not df_cp.empty:
        ...
        for g_name in pdf_cp_df['編組'].unique():
            ...
            if not sub.empty:
                first_code = sub.iloc[0]['無線電代號']
                pdf_cp_df.loc[pdf_cp_df['編組'] == g_name, '無線電代號'] = first_code
...
with tab2:
    ...
    if not res_cp_raw.empty:
        # 深度同步：將網頁當前打完的資料集透過全新演算法算好分組排序與2碼聯動代號
        res_cp = assign_cp_groups(res_cp_raw)
        
        # 【技術突破修正點】如果前端偵測到有變更（如手動改編組、或最下列新增列），
        # 立即主動進行 DataFrame 的精準重載同步並 rerun，徹底逼退 Streamlit 的舊畫面快取，代號立馬聯動更新！
