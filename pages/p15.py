...
def calculate_dynamic_stats(df_cmd, df_ptl, df_cp):
    ...
    sig_cmd = len(supervisors) if len(supervisors) > 0 else 7
    ...
    c_inv = 2
    return {'cmd': sig_cmd, 'ptl_机动': c_ptl, 'ptl_场所': c_cp, 'inv': c_inv, 'civ': 0, 'total': sig_cmd + c_ptl + c_cp + c_inv}
...
def generate_attendance_pdf(unit, project, time_str, stats):
    ...
    style_boss_mid = ParagraphStyle('BossMid', fontName=font, fontSize=14, leading=20, alignment=1, wordWrap='CJK')
