...
def save_data(unit, time_str, project, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    try:
        ...
        # 指揮/巡邏/臨檢 頁 (儲存前移除動態生成的組別欄位，維持資料庫乾淨)
        ...
        st.cache_data.clear() # 清除所有快取，強迫下次載入最新資料
        return True
    except Exception as e:
...
if df_set is None or df_set.empty:
    ...
    # 讀取時，若已有組別欄位，先予以移除，避免後續 data_editor 重複動態產生
...
with tab1:
    ...
    # 【安全修正】不使用 insert 修改原 DataFrame，而是動態整合生成全新變數
...
if st.button("💾 同步雲端並發送郵件", use_container_width=True):
    with st.spinner("⏳ 正在寫入雲端並寄送郵件，請稍候..."):
        # 這裡將動態生出的組別帶進 save_data，該函式內部已做好排除邏輯，不會污染 Google Sheets
