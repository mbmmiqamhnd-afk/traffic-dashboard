...
def calculate_dynamic_stats(df_cmd, df_ptl, df_cp):
    # 1. 督導組統計：依任務目標內含「督導」或「指導」自動撈取人員
    supervisors = set()
    for _, row in df_cmd.iterrows():
        ...
        if "督導" in aim or "指導" in aim:
            ...
            if leader_clean and leader_clean.lower() != "nan":
                supervisors.add(leader_clean)
            
            # 納入共同執行同仁（利用頓號、中英文逗號、頓號或空格拆分）
    ...
    c_inv = 3
    for _, row in df_cmd.iterrows():
        if "偵訊組" in str(row.get("項目", "")):
            c_inv = 2 # 依0410正本回歸2人基本值
...
st.subheader("貳、 警力統計及地點統計（系統完全自動精密統計）")
col_s1, col_s2, col_s3, col_s4, col_s5 = st.columns(5)
with col_s1: st.metric("督導組人數 (含共同)", value=f"{st.session_state.stats_data['cmd']} 人")
