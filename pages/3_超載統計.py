...
def send_auto_email(excel_bytes, subject_text):
    try:
        if "email" not in st.secrets:
            st.error("❌ 未偵測到 Email Secrets 設定，無法寄信。")
            return False
        ...
        msg['From'] = sender
        msg['To'] = sender # 預設寄回給自己，可修改為其他收件人
        
        body = "您好，附件為系統自動產生的超載統計報表，請查收。"
...
# 2. Google 試算表格式化指令
# ==========================================
def get_footer_red_request(ws_id, row_idx, col_idx, text):
    """僅將最後的百分比標紅"""
...
def get_header_red_request(ws_id, row_idx, col_idx, text):
    """標題列：數字符號紅，中文黑"""
...
if files and len(files) >= 3:
    try:
        ...
        st.markdown("---")
        if st.button("🚀 執行：同步試算表並自動寄送郵件", type="primary"):
            with st.status("正在執行自動化流程...") as s:
                # A. 產生 Excel 附件
                out = io.BytesIO()
                ...
                for i, col_t in enumerate(df_sync.columns[1:4], start=2):
                    reqs.append(get_header_red_request(ws.id, 2, i, col_t))
                f_idx = 2 + len(df_final) + 1
                reqs.append(get_footer_red_request(ws.id, f_idx, 1, f_plain))
                sh.batch_update({"requests": reqs})
                st.write("✅ 試算表同步與格式化成功")

                # C. 寄送郵件
                if send_auto_email(excel_data, f"🚛 超載統計報表 - {e_yt} (應達成率 {prog_str})"):
                    st.write("📧 電子郵件已成功寄出")
                
                st.balloons()
                s.update(label="全部流程已完成", state="complete")
