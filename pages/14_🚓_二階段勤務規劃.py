...
def load_data():
    try:
        ...
        # 嘗試讀取路檢組，如果沒有該分頁則回傳 None 讓後續使用預設值
        try:
...
def save_data(unit, time_str, project, briefing, station, df_cmd, df_ptl, df_cp):
    try:
        client = get_client()
...
# --- 3. PDF 生成功能 ---
def generate_pdf_from_data(unit, project, time_str, briefing, station, df_cmd, df_ptl, df_cp):
    font = _get_font()
    buf = io.BytesIO()
    ...
    t3.setStyle(TableStyle([('FONTNAME',(0,0),(-1,-1),font),('FONTSIZE',(0,0),(-1,-1),14),('ALIGN',(0,1),(1,-1),'CENTER'),
                            ('GRID',(0,0),(-1,-1),0.5,colors.black),('BACKGROUND',(0,0),(-1,0),colors.HexColor('#e6e6e6')),('VALIGN',(0,0),(-1,-1),'MIDDLE')]))
    story.append(t3)
...
# --- 4. 寄信功能 ---
def send_report_email(unit, project, time_str, briefing, station, df_cmd, df_ptl, df_cp):
    try:
        ...
        msg["Subject"] = f"二階段勤務規劃與簽到表_{datetime.now().strftime('%m%d')}"
        msg.attach(MIMEText("附件為最新的勤務規劃表與人員簽到表 PDF。", "plain", "utf-8"))
        
        pdf1 = generate_pdf_from_data(unit, project, time_str, briefing, station, df_cmd, df_ptl, df_cp)
...
tab1, tab2 = st.tabs(["📍 第一階段：機動巡邏 (21:00 - 22:30)", "🚧 第二階段：定點路檢及機動攔檢 (22:30 - 24:00)"])

with tab1:
    st.info("💡 系統會自動依據「單位」（優先看上方單位）及「服勤人員」計算無線電代號（所長為1、副所長為2）。")
...
with tab2:
    st.info("💡 此處為第二階段定點路檢與機動攔檢編組，同樣支援無線電代號自動帶入。")
...
def get_html():
    ...
    html += "<h4 style='margin-bottom:5px'>第一階段：機動巡邏 (21:00 - 22:30)</h4><table><tr><th>編組</th><th>代號</th><th>單位</th><th>人員</th><th>任務</th></tr>"
    ...
    html += "<h4 style='margin-bottom:5px'>第二階段：定點路檢及機動攔檢 (22:30 - 24:00)</h4><table><tr><th>編組</th><th>代號</th><th>單位</th><th>人員</th><th>任務</th></tr>"
...
pdf_plan = generate_pdf_from_data(u, p_name, p_time, b_info, s_info, res_cmd, res_ptl, res_cp)
...
if st.button("💾 同步雲端並發送備份郵件 (含兩份 PDF)", use_container_width=True):
    with st.spinner("同步中..."):
        save_data(u, p_time, p_name, b_info, s_info, res_cmd, res_ptl, res_cp)
        ok, mail_err = send_report_email(u, p_name, p_time, b_info, s_info, res_cmd, res_ptl, res_cp)
        if ok: st.success("✅ 同步成功，規劃表與簽到表已寄至信箱！(Google Sheets 已自動新增/更新「路檢組」工作表)")
