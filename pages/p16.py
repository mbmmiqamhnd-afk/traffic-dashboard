...
# 新增：郵件寄送功能
# ==========================================
def send_report_email(receiver_email, subject, content):
    """
    使用 Gmail SMTP 寄送報告
    """
    try:
        # 從 st.secrets 讀取設定，若本機測試請確保 secrets.toml 存在
        # 格式：
        # [email]
...
def d_extract_duty(d_file, hour):
    ...
    try:
        ...
        titles = {"分隊": {"A": "分隊長", "B": "小隊長", "C": "幹部"}, "隊": {"A": "隊長", "B": "副隊長", "C": "幹部"}, "所": {"A": "所長", "B": "副所長", "C": "幹部"}}
        default_titles = titles.get(unit_type, titles["所"])
        c_notes = []
        for code in ["A", "B", "C"]:
                    ...
                    if code in [d_normalize_code(x) for x in re.findall(r'[A-Za-z0-9]{1,2}', str(df.iloc[r, c]))]:
                        ...
                        if is_l: is_off = True
                            ...
                            for k, kn in zip(["巡", "守", "臨", "交", "路"], ["巡邏", "守望", "臨檢", "交整", "路檢"]):
                                if k in dt or k in str(df.iloc[r, c]): d_names.add(kn)
            ...
            if not found or is_off: c_notes.append(f"{full_name}休假")
                ...
                if p_slots:
                    ...
                    c_notes.append(f"{full_name}在{unit_type}督勤，編排{ms:02d}至{e_str}時段{'、'.join(sorted(d_names))}勤務")
...
def build_report(dr, er, u_time, d_s5_s, d_end_s, d_s3_s):
    ...
    lns = [
        ...
        f"{subject}手槍出勤 {eq['go']} 把、在{unit_type} {eq['gi']} 把，子彈出勤 {eq['bo']} 顆、在{unit_type} {eq['bi']} 顆，無線電出勤 {eq['ro']} 臺、在{unit_type} {eq['ri']} 臺；防彈背心出勤 {eq['vo']} 件、在{unit_type} {eq['vi']} 件，幹部對械彈每日檢查管制良好，符合規定{fix}。",
...
with main_tabs[1]:
    ...
    with u_tabs[-1]:
        if all_final_reports:
            full_text = "\n\n" + ("─" * 40) + "\n\n"
            full_text = full_text.join(all_final_reports)
            st.text_area("📄 總匯整結果", full_text, height=500, key="full_report")
            
            # 下載與郵件功能區
            st.divider()
            col_mail1, col_mail2 = st.columns([2, 1])
            with col_mail1:
                receiver = st.text_input("收件人信箱", placeholder="example@gmail.com", key="mail_to")
            with col_mail2:
                st.write("") # 調整按鈕高度
                if st.button("📧 寄送報告郵件", use_container_width=True):
                    if not receiver:
                        st.error("請先輸入收件人信箱！")
                    else:
                        subj = f"勤務督導報告匯整_{insp_date.strftime('%Y%m%d')}"
                        success, msg = send_report_email(receiver, subj, full_text)
