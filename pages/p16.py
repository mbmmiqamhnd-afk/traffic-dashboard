...
def d_extract_duty(d_file, hour):
    ...
    try:
        ...
        if target_col != -1:
            ...
            if "分隊" in res['unit_name']: titles = {"A": "分隊長", "B": "小隊長", "C": "小隊長"}
            c_notes = []
            for code in ["A", "B", "C"]:
                fname = f_map.get(code, titles[code])
                is_off = False
                for r in range(max(0, len(df)-8), len(df)):
                    if code in str(df.iloc[r, :]).upper() and any(k in "".join(df.iloc[r, :2]) for k in ["休","輪","假"]):
                        is_off = True; break
                ...
                for r in range(len(df)):
                    ...
                    if code in re.findall(r'[A-Z0-9]{1,2}', cell_val):
                        ...
                        kw_map = {"巡":"巡邏", "守":"守望", "望":"守望", "臨":"臨檢", "交":"交整", "路":"路檢", "專":"專案", "辦":"偵辦刑案"}
...
def d_extract_equip(e_file, hour):
    try:
        ...
        sub = df.iloc[:30]; sub_s = df_s.iloc[:30]
        def get_v(kw, k):
...
# 3. 主 UI 介面
# ==========================================
main_tabs = st.tabs(["📊 數據處理中心", "📋 勤務督導報告"])
...
with main_tabs[1]:
    ...
    with u_tabs[-1]:
        ...
        if reports_list:
            ...
            st.warning("目前尚無解析完成的報告。請先到各單位 Tab 上傳檔案。")
