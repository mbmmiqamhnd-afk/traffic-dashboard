...
# --- 1. 分頁基本配置 ---
st.set_page_config(page_title="督導報告 v8.1 - 單位自動辨識版", layout="wide")
...
st.title("📋 督導報告極速生成器 v8.1 (自動單位辨識)")
...
def extract_duty_logic(d_file, hour):
    ...
    try:
        ...
        for r in range(5):
            ...
            # 尋找包含 分局/派出所/分隊 等關鍵字，且過濾掉「勤務分配表」字眼
            match = re.search(r'([\u4e00-\u9fa5]+(分局|派出所|分隊|局))', row_text)
            if match:
                res['unit_name'] = match.group(1)
                break
        ...
        for r in range(5):
            ...
            for c_idx in range(len(df.columns)):
                ...
                if start_h is not None and end_h is not None:
                    temp_cols[c_idx] = (start_h, end_h); count += 1
            if count > max(len(time_cols), 3):
                time_row_idx = r; time_cols = temp_cols
...
for i in range(num_units):
    with tabs[i]:
        ...
        if duty_file and equip_file:
            ...
            u_name = duty_data['unit_name'] # 🌟 這裡使用自動抓取的單位名稱
