...
def parse_crime_pdf_gemini(pdf_file, roster: list, unit_idx: int) -> list:
    ...
    for i, img in enumerate(images):
        try:
            ...
            # 移除 markdown code fence（不使用 regex 反引號，避免截斷問題）
...
# 7. 報告組合
# ==========================================
def build_report(duty_info: dict, equip: dict, crimes: list, sup_time: str,
                 sup_date: str, supervisor: str) -> str:
    ...
    lines = []
    lines.append(f"督導單位：{unit}")
    lines.append(f"督導時間：{sup_date} {sup_time}")
    lines.append(f"督導人員：{supervisor}")
    lines.append("")

    # 值班
    lines.append(f"一、值班情形：{term}本時段由{v_name}擔任值班工作。")

    # 警備隊拘留
    if is_guard and detention:
        lines.append(f"二、拘留情形：本時段拘留業務由{detention}負責看管。")
        sec = 3
    else:
        sec = 2

    # 幹部
    lines.append(f"{_num(sec)}、幹部督勤：{cadre}")
    sec += 1

    # 天際線（僅派出所/分駐所）
    if has_skyline:
        lines.append(f"{_num(sec)}、天際線巡邏：{term}本時段天際線巡邏勤務執行正常，員警均依規定巡邏。")
        sec += 1

    # 裝備
    if equip:
        gun_i, gun_o = equip.get('gi', 0), equip.get('go', 0)
        bul_i, bul_o = equip.get('bi', 0), equip.get('bo', 0)
        rad_i, rad_o = equip.get('ri', 0), equip.get('ro', 0)
        vest_i, vest_o = equip.get('vi', 0), equip.get('vo', 0)
        lines.append(
            f"{_num(sec)}、警械裝備：在{loc_term}手槍 {gun_i} 枝、子彈 {bul_i} 發；"
            f"出勤手槍 {gun_o} 枝、子彈 {bul_o} 發；"
            f"無線電在{loc_term} {rad_i} 支、出勤 {rad_o} 支；"
            f"防彈背心在{loc_term} {vest_i} 件、出勤 {vest_o} 件，數量相符。"
        )
        sec += 1
    else:
        lines.append(f"{_num(sec)}、警械裝備：交接簿未上傳或解析失敗，請人工確認。")
        sec += 1

    # 刑案
    if crimes:
        lines.append(f"{_num(sec)}、查獲刑案：")
        for idx, c in enumerate(crimes, 1):
            suspect = c.get('嫌疑人', '不明')
            t = c.get('查獲時間', '不明')
            loc = c.get('查獲地點', '不明')
            law = c.get('觸犯法條', '不明')
            officer = c.get('查獲員警', '不明')
            lines.append(f"  ({idx}) {t}，{officer}於{loc}查獲{suspect}，涉嫌{law}。")
        sec += 1
    else:
        lines.append(f"{_num(sec)}、查獲刑案：本時段無刑案發生。")
        sec += 1

    lines.append("")
    lines.append("以上報告如實填報。")
    return "\n".join(lines)
...
def _num(n):
    ...
    return nums[n - 1] if 1 <= n <= 10 else str(n)
...
with st.expander("📋 督導基本資訊", expanded=True):
    ...
    with col2:
        sup_hour = st.number_input("督導時間（小時，24小時制）", min_value=0, max_value=23, value=datetime.now().hour)
    with col3:
        supervisor = st.text_input("督導人員姓名", value="")

sup_time_str = f"{sup_hour:02d}:00"
...
for i in range(num_units):
    with st.expander(f"第 {i+1} 個單位", expanded=(i == 0)):
        ...
        unit_inputs.append((d_file, e_file, p_file))
...
if st.button("🚀 生成督導報告", type="primary"):
    ...
    all_ready = all(d is not None for d, e, p in unit_inputs)
    if not all_ready:
        st.error("每個單位都必須上傳勤務表。")
        st.stop()

    st.session_state.unit_reports = {}
    progress = st.progress(0)

    for i, (d_file, e_file, p_file) in enumerate(unit_inputs):
        with st.spinner(f"正在處理第 {i+1} 個單位..."):
            # 勤務表解析
            duty_info = extract_duty_v2(io.BytesIO(d_file.read()), sup_hour)
            ...
            report_text = build_report(
                duty_info, equip, crimes,
                sup_time_str, sup_date_str, supervisor
            )
...
if st.session_state.unit_reports:
    ...
    st.markdown("---")
    all_text = "\n\n" + "="*40 + "\n\n"
    all_text = all_text.join([v['report'] for v in st.session_state.unit_reports.values()])
    st.download_button(
        label="⬇️ 下載全部報告（合併）",
