...
def sort_coworkers(df):
    ...
    def get_rank_weight(row):
        title = str(row['職別'])
        unit = str(row['單位'])
        # 【關鍵修正】：如果單位是交通組，不論主管或承辦人，在排序與分錢階段一律視為同等階，避免因排序領跑拿到不同金額
        if '交通組' in unit:
            return 7
...
def p18_page():
    ...
    st.info("🔥 系統更新：交通組主管與同仁之分配基準已完全同步，金額計算將達到完全一致。")
    ...
    if "系統自動" in alloc_mode:
        st.info("💡 負責管考(72%)：正副主官固定8%（分局長60%、副分局長40%），其餘按56%：26%：10%比例分配。交通組全體人員金額已完全對齊。")
