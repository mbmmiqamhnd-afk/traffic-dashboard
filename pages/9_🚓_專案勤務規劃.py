...
# ✅ 修復：原本用 mm 當變數名稱，與 reportlab 的 mm 單位衝突，已改為 month_str / day_str
match = re.search(r"(\d+)月(\d+)日", p_time)
if match:
    month_str = str(match.group(1)).zfill(2)
    day_str   = str(match.group(2)).zfill(2)
    date_prefix = f"{month_str}{day_str}"
