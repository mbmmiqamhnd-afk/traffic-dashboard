...
def generate_pdf(unit, project, time_str, briefing, station, df_cmd, df_ptl):
    font = _get_font()
    buf = io.BytesIO()
    ...
    s_title  = ParagraphStyle("t",  fontName=font, fontSize=14, alignment=1, spaceAfter=2, leading=18)
    s_info   = ParagraphStyle("i",  fontName=font, fontSize=12, alignment=2, spaceAfter=4)
    ...
    s_header = ParagraphStyle("h",  fontName=font, fontSize=14, alignment=1, leading=18)

    def c(txt, style=None):
        txt = str(txt).replace("\n","<br/>").replace("、","<br/>").replace(",","<br/>")
        return Paragraph(txt, style or s_cell)

    story.append(Paragraph(f"{unit}執行{project}規劃表", s_title))
    story.append(Paragraph(f"勤務時間：{time_str}", s_info))
    story.append(Spacer(1, 2*mm))

    # 任務編組表
    cw1 = [W*0.14, W*0.13, W*0.26, W*0.47]
    ...
    story.append(Paragraph(f"<b>🚧 {station.replace(chr(10),'<br/>')}</b>", s_note))
    story.append(Spacer(1, 3*mm))

    # 巡邏組表
    cw2 = [W*0.10, W*0.08, W*0.12, W*0.18, W*0.52]
    data2 = [[c("<b>編組</b>"), c("<b>代號</b>"), c("<b>單位</b>"), c("<b>服勤人員</b>"), c("<b>任務分工</b>")]]
    for _, row in df_ptl.iterrows():
        task = str(row.get('任務分工','')) + "\n*雨備方案：轄區治安要點巡邏。"
        data2.append([c(row.get('編組','')), c(row.get('無線電','')),
                      c(row.get('單位','')), c(row.get('服勤人員','')),
                      c(task, s_left)])
...
def send_report_email(html_content, subject, unit, project, time_str, briefing, station, df_cmd, df_ptl):
    import urllib.parse as _ul
    try:
        sender   = st.secrets["email"]["user"]
        password = st.secrets["email"]["password"]
        receiver = sender
        pdf_bytes = generate_pdf(unit, project, time_str, briefing, station, df_cmd, df_ptl)
        msg = MIMEMultipart()
        msg["From"]    = sender
        msg["To"]      = receiver
        msg["Subject"] = subject
