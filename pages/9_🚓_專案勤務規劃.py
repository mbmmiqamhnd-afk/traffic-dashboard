...
def get_client():
    if "gcp_service_account" not in st.secrets:
        # 如果沒有設定 secrets，回傳 None，讓後面走離線模式
        return None
...
def load_data():
    try:
        client = get_client()
        if client is None:
            return None, None, None, "未設定 Google Sheets API (Secrets)"
        ...
        if not all([ws_set, ws_cmd, ws_ptl]):
            return None, None, None, "Google Sheets 中缺少必要的工作表"
...
def send_report_email(html_content, subject, unit, time_str, project, briefing, station, df_cmd, df_ptl):
    ...
    try:
        ...
        if pdf_bytes is None:
            return False, "PDF 生成失敗 (請檢查 kaiu.ttf 字型是否上傳)"
