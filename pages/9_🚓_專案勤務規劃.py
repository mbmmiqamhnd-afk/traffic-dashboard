...
def get_client():
    ...
    if "gcp_service_account" not in st.secrets:
        raise ValueError("secrets.toml 中缺少 gcp_service_account 設定")
...
def load_data():
    try:
        ...
        if not all([ws_set, ws_cmd, ws_ptl]):
            return None, None, None, "Google Sheets 中缺少必要的工作表 (設定, 指揮組, 巡邏組)"
...
def generate_pdf_from_data(unit, project, time_str, briefing, station, df_cmd, df_ptl):
    ...
    data_cmd = []
    title_row = [Paragraph("<b>任　務　編　組</b>", style_cell)] # ReportLab 表頭處理較繁瑣，這裡簡化
