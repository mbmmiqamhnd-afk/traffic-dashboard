...
with st.expander("📌 專案公文重點提示與待辦時限", expanded=False):
    ...
    * **車種敘獎標準精確規範**：
      ...
      4. **其餘慢車（電輔車、腳踏自行車）**：不納入本次專案敘獎計點。
      5. **人事獎懲拆解**：非大型車累積點數精確拆解為 **「嘉獎二次」**（商數 // 2）與 **「嘉獎一次」**（餘數 % 2）。
    """)
...
if uploaded_file:
    ...
    if fact_col and vehicle_col:
        ...
        with st.expander("🛠️ 點此確認或微調各車種折算點數（微電車已自動納入機車）", expanded=True):
            ...
            heavy_pts = s1.number_input("🚛 大型車毒駕每件記功次數：", min_value=1, max_value=2, value=1, step=1, help="大型車每件核給記功一次")
            car_pts = s2.number_input("🚗 小型車毒駕每件嘉獎點數：", min_value=1, max_value=3, value=2, step=1, help="小型車每件核給2點（1個嘉獎二次）")
            moto_pts = s3.number_input("🛵 機車(含微電車)每件點數：", min_value=1, max_value=2, value=1, step=1, help="機車及微型電動二輪車每件核給1點")
            s4.info("🚲 其餘慢車（電輔車、自行車等）依規定不納入專案敘獎計點。")
        ...
        with tab_officer:
            ...
            if officer_col:
                ...
                for officer in officer_list:
                    ...
                    moto_cnt = len(sub[sub["車種判定"] == "機車"])  # 包含重機、輕機、微型電動二輪車
                    ...
                    # 2. 其他車種：累計嘉獎點數 (小型車2點、機車含微電車1點，其餘慢車不計)
