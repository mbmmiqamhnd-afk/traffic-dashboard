...
DEFAULT_PTL_DESC = "巡邏勤務：20時至24時，機動巡邏與交通稽查"

EXPECTED_PTL_COLS = ["無線電代號", "單位", "服勤人員", "巡邏路段"]
...
def load_data():
    try:
        client = get_client()
        if client is None: return None, None, None, "權限不足或未設定密鑰"
...
def save_data(unit, time_str, project, briefing, df_cmd, df_ptl, ptl_desc):
    try:
        client = get_client()
...
# --- PDF 相關函數 ---
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, ptl_desc):
    font = _get_font()
    buf = io.BytesIO()
    ...
    story.append(Paragraph(f"<b>{ptl_desc}</b>", style_middle_block))
    
    # 根據新的 4 欄位進行合併樣式判定
    span_styles_ptl = get_merge_styles(df_ptl, ["無線電代號", "單位", "巡邏路段"])
    ...
    # 調整 PDF 欄寬比例 (共100%)：無線電15%、單位15%、人員35%、路段35%
    t2 = Table(data_ptl, colWidths=[page_width*0.15, page_width*0.15, page_width*0.35, page_width*0.35])
    base_style_ptl = [
        ('FONTNAME',(0,0),(-1,-1),font),
        ('GRID',(0,0),(-1,-1),0.5,colors.black),
...
def send_report_email(unit, project, time_str, briefing, df_cmd, df_ptl, ptl_desc):
    try:
        ...
        p1 = generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, ptl_desc)
...
def auto_assign_radio_code(df):
    ...
    for idx, row in df_copy.iterrows():
        ...
        personnel = safe_str(row.get('服勤人員')).strip()
        current_radio = safe_str(row.get('無線電代號')).strip()
        
        # 整理文字用於判斷
        clean_unit = re.sub(r'\s+', '', unit)
        clean_personnel = re.sub(r'\s+', '', personnel)
        
        # 如果該列沒有代號，才嘗試自動填寫
        if current_radio == "":
            base_pfx = next((v for k, v in base_prefixes.items() if k in clean_unit), "")
            if base_pfx:
                # 邏輯判斷：服勤人員字眼包含所長/分隊長 -> 1, 副所長/小隊長 -> 2, 其餘 -> 0
                if any(x in clean_personnel for x in ["所長", "分隊長"]):
                    suffix = "1"
                elif any(x in clean_personnel for x in ["副所長", "小隊長"]):
                    suffix = "2"
                else:
                    suffix = "0"
                
                df_copy.at[idx, '無線電代號'] = f"隆安{base_pfx}{suffix}"
...
if isinstance(df_ptl, pd.DataFrame) and not df_ptl.empty:
    # 新增相容: 將可能出現的舊名稱轉移
    if '姓名' in df_ptl.columns and '服勤人員' not in df_ptl.columns:
        df_ptl['服勤人員'] = df_ptl['姓名']
    if '任務分工' in df_ptl.columns and '服勤人員' not in df_ptl.columns:
        df_ptl['服勤人員'] = df_ptl['任務分工']
...
p1_d = d.get("ptl_desc",       DEFAULT_PTL_DESC)
...
if st.button("💾 同步雲端並發送 Email 備份", use_container_width=True):
    with st.spinner("同步中，請稍候…"):
        if save_data(u, p_time, p_name, b_info, res_cmd, res_ptl, phase1_desc):
            with st.spinner("同步成功，正在寄送郵件…"):
                ok, mail_err = send_report_email(u, p_name, p_time, b_info, res_cmd, res_ptl, phase1_desc)
            if ok:
