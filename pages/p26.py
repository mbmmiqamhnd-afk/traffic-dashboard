...
def main():
    ...
    st.info("本系統整合了「交辦單產生器」與「護老專案時數統計與報表匯出」功能。")
    ...
    with tab1:
        ...
        st.write("依據警察局來文指示，自動產出各單位之正式交辦單內容[cite: 1]。")

        units = ["龍潭所", "聖亭所", "中興所", "石門所", "高平所", "勤務指揮中心", "龍潭交通分隊"]
        selected_unit = st.selectbox("選擇受文單位：", units)
...
辦理期限：115年7月28日前辦理完畢連同原件具報[cite: 1]。
        ...
        st.text_area("交辦單預覽內容：", value=notice_content.strip(), height=350)
        st.download_button(
            label=f"📥 下載【{selected_unit}】交辦單文字檔",
            data=notice_content.strip(),
