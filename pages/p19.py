...
DEFAULT_PROJ    = "0325「雷霆除暴專案」暨自辦場所臨檢與取締酒後駕車二合一專案"
...
DEFAULT_PTL = pd.DataFrame([
    {"組別": "第1路檢組", "無線電代號": "隆安51", "派遣單位": "聖亭所", "職別": "所長", "姓名": "鄭榮捷", "任務分工": "帶班兼管制", "攜行裝裝備": "槍彈、無線電、小電腦、密錄器", "臨檢目標": "北龍路319號前\n（攔檢中興路往龍潭市區方向）\n20時20分分局一樓集合出發臨檢"},
...
# ★ 修正大核心：直寫式獨立同步函數，徹底免除 NameError
...
if st.button("💾 同步雲端並發送郵件", use_container_width=True):
    if save_data(u, p_time, p_name, DEFAULT_BRIEF, res_cmd, res_ptl, res_cp, current_stats, res_ptl_focus, res_cp_focus):
        ...
        if ok: st.success("✅ 二合一專案資料已 100% 強制同步至二合一分頁！")
