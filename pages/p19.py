...
st.set_page_config(page_title="二合一勤務規劃系統", layout="wide", page_icon="🚓")
...
# 第一階段改為定點路檢的預設提示與欄位
DEFAULT_PTL_FOCUS = "落實定點路檢站部署，針對酒駕、改裝噪音車攔停盤查。若攔獲疑似噪音車輛，立即引導至「警政大樓廣場」交由環保局檢驗。"
DEFAULT_CP_FOCUS = "由第一階段之部分機動警力，會合偵查隊專案人員，於21時30分準時進入目標場所執行威力掃蕩。"
...
DEFAULT_PTL = pd.DataFrame([
    {"單位": "聖亭所", "服勤人員": "所長 鄭榮捷\n警員 詹宗澤", "任務分工": "帶班\n盤查兼警戒", "攜行裝備": "槍彈、無線電", "路檢地點與責任區": "中正路與大昌路口 (定點路檢)"},
    {"單位": "龍潭所", "服勤人員": "所長 孫祥愷\n警員 沈庭禾", "任務分工": "盤查兼警戒", "攜行裝備": "槍彈、無線電", "路檢地點與責任區": "北龍路與金龍路口 (定點路檢)"},
])
...
def save_data(unit, time_str, project, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    try:
        ...
        # 設定頁
        try: ws_set = sh.worksheet("二階段_設定")
        except: ws_set = sh.add_worksheet(title="二階段_設定", rows="50", cols="5")
        ws_set.clear()
        ...
        # 指揮/路檢/臨檢 頁面資料清洗儲存
        for ws_name, df in [("二階段_指揮組", df_cmd), ("二階段_巡邏組", df_ptl), ("二階段_路檢組", df_cp)]:
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    data_stats = [[Paragraph("督導組", style_cell), Paragraph("路檢組", style_cell), Paragraph("偵訊組", style_cell), Paragraph("小計", style_cell), Paragraph("民力", style_cell), Paragraph("總計", style_cell)], 
    ...
    df_cmd_clean = clean_df(df_cmd)
    for _, r in df_cmd_clean.iterrows():
    ...
    df_ptl_clean = clean_df(df_ptl)
    p1_addr_col = '路檢地點與責任區' if '路檢地點與責任區' in df_ptl_clean.columns else '巡邏與攔查責任區'
    
    data_ptl = [[Paragraph(f"<b>{h}</b>", style_cell) for h in ["組別", "單位", "服勤人員", "任務分工", "攜行裝備", "路檢地點與責任區"]]]
    for _, r in df_ptl_clean.iterrows():
        data_ptl.append([Paragraph(clean(r.get('組別')), style_cell), Paragraph(clean(r.get('單位')), style_cell), Paragraph(clean(r.get('服勤人員')), style_cell_left), Paragraph(clean(r.get('任務分工')), style_cell_left), Paragraph(clean(r.get('攜行裝備')), style_cell_left), Paragraph(clean(r.get(p1_addr_col)), style_cell_left)])
...
def send_report_email(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    try:
        ...
        msg.attach(MIMEText("附件為最新版本勤務規劃表與簽到表。", "plain"))
        ...
        part1.add_header("Content-Disposition", f"attachment; filename*=UTF-8''{_ul.quote(f'{unit}二階段規劃表.pdf')}")
        msg.attach(part1)
        ...
        part2.add_header("Content-Disposition", f"attachment; filename*=UTF-8''{_ul.quote(f'{unit}二階段簽到表.pdf')}")
        msg.attach(part2)
...
if err or df_set is None:
    ...
    if not df_ptl.empty:
        ...
        if "巡邏與攔查責任區" in ed_ptl.columns:
            ed_ptl = ed_ptl.rename(columns={"巡邏與攔查責任區": "路檢地點與責任區"})
    else:
...
c_cmd, c_ptl, c_inv, c_civ = col_s1.number_input("督導組", value=default_stats['cmd']), col_s2.number_input("路檢組 (第一階段)", value=default_stats['ptl']), col_s3.number_input("偵訊組", value=default_stats['inv']), col_s4.number_input("民力", value=default_stats['civ'])
...
with tab1:
    ...
    if not res_ptl.empty: res_ptl.insert(0, "組別", [f"第{i+1}路檢組" for i in range(len(res_ptl))])
with tab2:
...
if st.button("💾 同步雲端並發送郵件", use_container_width=True):
    with st.spinner("同步雲端與寄送郵件中..."):
        if save_data(u, p_time, p_name, b_info, res_cmd, res_ptl, res_cp, current_stats, p_ptl_focus, p_cp_focus):
            ...
            if ok: st.success("✅ 已同步並成功寄出郵件！")
            else: st.warning(f"⚠️ 雲端資料同步成功，但郵件寄送失敗: {mail_err}")
