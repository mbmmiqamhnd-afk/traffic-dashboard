...
# 獨立二合一系統唯一指定的 4 張 Google Sheets 工作表分頁名稱（強力鎖死）
WS_SET_NAME = "二合一_設定"
WS_CMD_NAME = "二合一_指揮組"
WS_PTL_NAME = "二合一_路檢組"
...
DEFAULT_CMD = pd.DataFrame([
    ...
    {"項目": "上級督導官", "通訊代號": "建興", "任務目標": "重點機元督導", "負責人員": "督察 孫三陽", "共同執行人員": ""},
...
# ★ 修正重點：解除批次對稱迴圈，全面改用單獨直讀寫死路由，保證 100% 寫入二合一工作表分頁
def save_data(unit, time_str, project, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    try:
        ...
        ws_c.clear()
        if not df_cmd.dropna(how='all').empty:
            ws_c.update(range_name='A1', values=[df_cmd.columns.tolist()] + df_cmd.dropna(how='all').fillna("").astype(str).values.tolist())

        # 直寫路檢組
        try: ws_p = sh.worksheet(WS_PTL_NAME)
        except: ws_p = sh.add_worksheet(title=WS_PTL_NAME, rows="100", cols="20")
        ws_p.clear()
        if not df_ptl.dropna(how='all').empty:
            ws_p.update(range_name='A1', values=[df_ptl.columns.tolist()] + df_ptl.dropna(how='all').fillna("").astype(str).values.tolist())

        # 直寫擴大臨檢組
        try: ws_cp = sh.worksheet(WS_CP_NAME)
        except: ws_cp = sh.add_worksheet(title=WS_CP_NAME, rows="100", cols="20")
        ws_cp.clear()
        if not df_cp.dropna(how='all').empty:
            ws_cp.update(range_name='A1', values=[df_cp.columns.tolist()] + df_cp.dropna(how='all').fillna("").astype(str).values.tolist())
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    # 職別(0.10)與姓名(0.11)寬度已最佳化，容納三個字綽綽有餘
...
if st.button("💾 同步雲端並發送郵件", use_container_width=True):
    if save_data(u, p_time, p_name, DEFAULT_BRIEF, res_cmd, res_ptl, res_cp, current_stats, res_ptl_focus, res_cp_focus):
        ...
        st.error("❌ 同步失敗，請確認 Google Sheets 後台分頁存在。")
