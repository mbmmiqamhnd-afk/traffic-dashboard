...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    # 🎯 配合定點路檢的五欄位規格動態產出對應表頭與寬度
    headers = ["組別", "無線電", "單位", "服勤人員", "任務分工"]
    ...
    for _, r in df_ptl_clean.iterrows():
        ...
        data_ptl.append([
            ...
            Paragraph(clean(code), style_cell), 
            Paragraph(clean(r.get('單位')), style_cell), 
            Paragraph(clean(r.get('服勤人員')), style_cell_left), 
            Paragraph(clean(分工), style_cell_left)
        ])
    t_ptl = Table(data_ptl, colWidths=[page_width*0.11, page_width*0.12, page_width*0.15, page_width*0.25, page_width*0.37])
...
if err or df_set is None:
    ...
    # 🎯 雲端資料加載時，針對第一階段進行舊欄位到定點路檢欄位的名稱轉換與清洗
    if not df_ptl.empty:
        ed_ptl = df_ptl.copy()
        ...
        if "攜行裝備" in ed_ptl.columns:
            ed_ptl = ed_ptl.drop(columns=["攜行裝備"])
        if "無線電" not in ed_ptl.columns:
            ed_ptl.insert(0, "無線電", "")
        if "編組" not in ed_ptl.columns:
            ed_ptl.insert(0, "編組", "")
    else:
...
with tab1:
    # 🎯 這裡只針對第一階段定點路檢進行調整：Data Editor 輸出後動態生成組別名稱
