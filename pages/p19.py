...
# ★ 獨立二合一系統唯一指定的 4 張 Google Sheets 工作表分頁名稱（全面鎖死）
WS_SET_NAME = "二合一_設定"
WS_CMD_NAME = "二合一_指揮組"
WS_PTL_NAME = "二合一_路檢組"
...
DEFAULT_CHECKPOINT = pd.DataFrame([
    ...
    {"組別": "第2臨檢組", "無線電代號": "隆安82", "派遣單位": "忠興所",   "職別": "警員",   "姓名": "江益德", "任務分工": "盤查兼蒐證",                      "臨檢目標場所": "A. 鉅大撞球館（中豐路558號）IC329\nB. 台灣麻將協會（中豐路558之1號）IC328\nF. 憤怒鳥網咖（中興路269號）IB330\nG. 真情男女養生館（中興路387號）IB329\nH. 萬紫千紅舒壓館（中興路491-3號）IB326"},
...
@st.cache_resource
def get_client():
    """建立並快取 gspread 客戶端。需在 Streamlit Secrets 設定 [gcp_service_account]。"""
    try:
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    data_stats = [
        ...
        [
            ...
            Paragraph(str(stats["cmd"]+stats["ptl"]+stats["inv"]), style_cell),
    ...
    t_cmd = Table(data_cmd, colWidths=[
        page_width*0.14, page_width*0.16, page_width*0.3,
...
if err or df_set is None:
    ...
    ed_ptl = df_ptl[PTL_COLS] if (not df_ptl.empty and all(c in df_ptl.columns for c in PTL_COLS)) else DEFAULT_PTL.copy()
    ed_cp  = df_cp[CP_COLS]   if (df_cp is not None and not df_cp.empty and all(c in df_cp.columns for c in CP_COLS))  else DEFAULT_CHECKPOINT.copy()
...
c_cmd = col_s1.number_input("督導組", value=default_stats["cmd"], min_value=0)
c_ptl = col_s2.number_input("攔臨組", value=default_stats["ptl"], min_value=0)
