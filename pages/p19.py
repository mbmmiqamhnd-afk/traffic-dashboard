...
# ★ 獨立二合一系統唯一指定的 4 張 Google Sheets 工作表分頁名稱（鎖死路由）
WS_SET_NAME = "二合一_設定"
WS_CMD_NAME = "二合一_指揮組"
WS_PTL_NAME = "二合一_路檢組"
...
# ★ 修正點：預設專案名稱完全移除三合一字眼，全面改為二合一專案
DEFAULT_PROJ    = "0529「全國同步擴大取締酒後駕車及防制危險駕車」暨「擴大臨檢」二合一專案"
...
# ★ 獨立二合一後台資料同步儲存（鎖死只同步到二合一分頁，絕不回流三合一）
def save_data(unit, time_str, project, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    try:
        ...
        # 批次強力寫入二合一專屬各分頁
        for ws_name, df in [(WS_CMD_NAME, df_cmd), (WS_PTL_NAME, df_ptl), (WS_CP_NAME, df_cp)]:
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    # ★ 職別(0.10)與姓名(0.11)擴大欄寬，支援容納至少三個中文字
...
if st.button("💾 同步雲端並發送郵件", use_container_width=True):
    if save_data(u, p_time, p_name, DEFAULT_BRIEF, res_cmd, res_ptl, res_cp, current_stats, res_ptl_focus, res_cp_focus):
        ...
        if ok: st.success("✅ 二合一專案資料已 100% 鎖定同步至二合一分頁，不再回流三合一！")
        else:  st.warning(f"⚠️ 同步成功但郵件失敗: {mail_err}")
    else:
        st.error("❌ 同步失敗，請確認 Google Sheets 二合一分頁存在且權限正常。")
