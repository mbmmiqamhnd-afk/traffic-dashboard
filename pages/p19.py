...
# ★ 預設資料全面對齊 0325 雷霆除暴專案計畫表
DEFAULT_UNIT    = "桃園市政府警察局龍潭分局"
DEFAULT_TIME    = "115年3月25日 18時至22時"
DEFAULT_PROJ    = "0325「雷霆除暴專案」暨自辦擴大臨檢與取締酒後駕車專案"
DEFAULT_BRIEF   = "一、 落實三安：同仁執行盤查、臨檢及路檢勤務過程中，應強化敵情觀念，提高危機意識，並特別注意人犯戒護，落實「人犯戒護安全、案件程序安全、執法者及民眾安全」。\n二、 臨檢合法性：除法律另有規定外，警察人員執行場所之臨檢，應限於已發生危害或依客觀合理判斷易生危害之場所、交通工具或公共場所為之。\n三、 攔停規範：确实依司法院大法官釋字第535號解釋及「警察職權行使法」相關規定，注意遵守比例原則及考量民眾觀觀感，不得逾越必要程度。\n四、 全程蒐證：執行各項干涉、取締、處理糾紛及爭議性勤務，務必全程連續錄音或錄影。\n五、 異議處理：民眾對警察行使職權表示異議，認為無理由者得繼續執行，但經請求時應將異議之理由製作紀錄交付之。"
...
# ★ 導入 0325 專案真實督導及其他任務編組表 (共 12 列)
DEFAULT_CMD = pd.DataFrame([
    {"項目": "指揮官", "通訊代號": "隆安 1 號", "任務目標": "勤務核定並重點機動督導", "負責人員": "分局長 施宇峰", "共同執行人員": "秘書 陳鵬翔、警員 張庭溱"},
...
# ★ 導入 0325 專案真實【第一階段】定點路檢任務編組 (共 11 列)
...
# ★ 導入 0325 專案真實【第二階段】擴大臨檢任務編組 (第 1、2 臨檢組完整對齊，共 13 列)
DEFAULT_CHECKPOINT = pd.DataFrame([
    # 第1臨檢組 (隆安51) - 包含 5 處目標場所
...
def load_data():
    try:
        ...
        # 依據歷史專案名稱，全面動態路由「路檢組」與「擴大臨檢組」的工作表分頁
        ptl_ws_name = "三合一_巡邏組"
...
# 依據計畫表第1頁與第4頁，對齊 0325 專案真實統計初始化數據
default_stats = {
