...
DEFAULT_PROJ    = "0325雷霆除暴專案暨自辦擴大臨檢與取締酒後駕車專案"
...
DEFAULT_PTL_FOCUS = "於各定點實施路檢，針對酒駕熱點攔停盤查；攔獲疑似改裝噪音車，立即引導至「警政大樓廣場」交由環保局檢驗。"
DEFAULT_CP_FOCUS = "由第一階段之各組機動警力，會合偵查隊專案人員，準時進入目標場所執行威力掃蕩。"

DEFAULT_CMD = pd.DataFrame([
    {"項目": "指揮官", "通訊代號": "隆安 1 號", "任務目標": "勤務核定並重點機動督導", "負責人員": "分局長 施宇峰", "共同執行人員": "秘書 陳鵬翔"},
    ...
    {"項目": "督察組", "通訊代號": "隆安 6 號", "任務目標": "機動督導第二階段時檢組", "負責人員": "督察組長 黃長旗", "共同執行人員": "警務員 陳冠彰"},
    {"項目": "交通組", "通訊代號": "隆安 13號", "任務目標": "機動督導第一階段路檢組", "負責人員": "交通組長 楊孟竟", "共同執行人員": "巡官 郭勝隆"},
])
...
# ★ 補齊並校正真實專案的「第1臨檢組」5處地點與「第2臨檢組」完整編制資料
DEFAULT_CHECKPOINT = pd.DataFrame([
    ...
    {"組別": "第1臨檢組", "無線電代號": "隆安51", "派遣單位": "偵查隊", "職別": "偵查佐", "姓名": "賴享宏", "任務分工": "刑案偵防、社維法案件處理及移送", "臨檢目標場所": "A. 鉅大撞球館（中豐路558號）IC329\nB. 台灣麻將協會（中豐路558之1號）IC328\nC. 丹陽泰養生館（中豐路281號）IC335\nD. 溫馨汽車旅館（中正路457號）IA337\nE. 凱虹汽車旅館（中正路506號）IA318"},
    {"組別": "第1臨檢組", "無線電代號": "隆安51", "派遣單位": "偵查隊", "職別": "警員",   "姓名": "張峻銨", "任務分工": "刑案偵防、社維法案件處理及移送", "臨檢目標場所": "A. 鉅大撞球館（中豐路558號）IC329\nB. 台灣麻將協會（中豐路558之1號）IC328\nC. 丹陽泰養生館（中豐路281號）IC335\nD. 溫馨汽車旅館（中正路457號）IA337\nE. 凱虹汽車旅館（中正路506號）IA318"},
    # 第2臨檢組 (隆安82)
    ...
    {"組別": "第2臨檢組", "無線電代號": "隆安82", "派遣單位": "偵查隊", "職別": "警員",   "姓名": "駿宏",   "任務分工": "刑案偵防、社維法案件處理及移送", "臨檢目標場所": "A. 鉅大撞球館（中豐路558號）IC329\nB. 台灣麻將協會（中豐路558之1號）IC328\nF. 憤怒鳥網咖（中興路269號）IB330\nG. 真情男女養生館（中興路387號）IB329\nH. 萬紫千紅舒壓館（中興路491-3號）IB326"}
])
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    # ★ 修正點：獨立為第二階段長文字目標場所設計精細樣式，縮小字型、緊湊行高
    ...
    # ★ 5. 第二階段（擴大臨檢）— 重新調配欄寬比例，加大目標欄位至40%空間，並加入拆列參數防截斷
    ...
    if df_cp is not None and not df_cp.empty:
        ...
        for i, r in rows_cp.iterrows():
            ...
            data_cp.append([
                ...
                Paragraph(clean(r.get('臨檢目標場所','')), style_cp_target), # 使用優化後的長文字專用樣式
            ])
            
        if cp_prev_group is not None:
            cp_merge_groups.append((cp_grp_start, len(rows_cp)))

        # ★ 關鍵修正：加入 splitByRow=True，確保大資料量在跨頁時以列為單位切斷，防止整組被吞掉
...
if err or df_set is None:
    ...
    default_stats.update({
        'cmd':    int(d.get("stats_cmd",  6)),
        'ptl':    int(d.get("stats_ptl",  39)),
        'inv':    int(d.get("stats_inv",  2)),
