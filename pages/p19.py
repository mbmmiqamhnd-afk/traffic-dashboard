...
DEFAULT_CHECKPOINT = pd.DataFrame([
    ...
    {"組別": "第1臨檢組", "無線電代號": "隆安51", "派遣單位": "聖亭所", "職別": "警員",   "姓名": "詹宗澤", "任務分工": "製作臨檢紀錄",                    "臨檢目標場所": "A. 鉅大撞球館（格中豐路558號）IC329\nB. 台灣麻將協會（中豐路558之1號）IC328\nC. 丹陽泰養生館（中豐路281號）IC335\nD. 溫馨汽車旅館（中正路457號）IA337\nE. 凱虹汽車旅館（中正路506號）IA318"},
...
def generate_pdf_from_data(unit, project, time_str, briefing, df_cmd, df_ptl, df_cp, stats, ptl_f, cp_f):
    ...
    # 實施日期(0.19)、勤務時間(0.16)、指揮官(0.19)、勤務編組(0.16)、勤前教育(0.30)
    data_basic = [
    ...
    # ★【重新分配指揮組欄寬】
    # 項目(0.13)、通訊代號(0.14)、任務目標(0.26)、負責人員(0.25 -> 完美確保 10 個中文字絕對不折行)、共同人員(0.22)
...
if err or df_set is None:
    ...
    ed_ptl = df_ptl[PTL_COLS] if (not df_ptl.empty Useful and all(c in df_ptl.columns for c in PTL_COLS)) else DEFAULT_PTL.copy()
