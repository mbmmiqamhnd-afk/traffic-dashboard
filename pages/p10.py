...
def ensure_data_loaded():
    if "sheets_data" not in st.session_state:
        set_df, cmd_df, ptl_df, settings, err = load_data_from_api()
        if err:
            st.session_state["sheets_data"] = {
                "set_df": None, "cmd_df": None, "ptl_df": None, "settings": {}, "err": err
            }
        else:
            st.session_state["sheets_data"] = {
                "set_df": set_df, "cmd_df": cmd_df, "ptl_df": ptl_df, "settings": settings, "err": None
            }
...
if err:
    st.warning(f"⚠️ 無法連線 Google Sheets（{err}），顯示預設底稿。請稍候點擊側邊欄【強制重新載入】再試。")
