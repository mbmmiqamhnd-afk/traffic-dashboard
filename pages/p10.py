...
# =========================
# 字體、Google Sheets、PDF 等函數（省略以節省空間，請保留您之前的版本）
# =========================
# ... _get_font(), get_client(), init_sheets(), load_data(), save_data(), generate_pdf(), send_email() ...
...
# 關鍵設定：使用 num_rows="fixed" 徹底移除多餘空白行
res_ptl = st.data_editor(
    use_ptl, 
    num_rows="fixed",           # 這是重點！不允許自動新增空白行
    use_container_width=True, 
    height=480                  # 適當高度，避免多餘空間
)
