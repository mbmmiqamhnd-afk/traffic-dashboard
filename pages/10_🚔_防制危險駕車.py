...
def generate_pdf_from_data(time_str, commander, df_cmd, df_patrol):
    ...
    style_cell = ParagraphStyle('Cell', fontName=font, fontSize=14, leading=16, alignment=1) # 縮小 leading 讓並列更緊湊
    ...
    for _, r in df_patrol.iterrows():
        data_ptl.append([Paragraph(clean(r.get('勤務時段','')), style_cell), clean(r.get('無線電','')),
                         ...
                         Paragraph(clean(r.get('服勤人員','')), style_cell), # 這裡會自動套用換行
...
# 魔法連動：指揮官變更時自動拆分第一筆服勤人員
if len(ed_ptl) > 0:
    m_unit = re.search(r'([\u4e00-\u9fa5]+(?:所|分隊|分局))(.*)', cmdr_input)
    if m_unit:
        unit_name, title_name = m_unit.group(1), m_unit.group(2).strip()
        # 更新第一筆服勤人員：時段與姓名垂直並列
        ppl_text = str(ed_ptl.loc[0, '服勤人員'])
        if title_name and title_name not in ppl_text:
            segments = re.findall(r'(\d{2}-\d{2}時)', ppl_text)
            if segments:
                new_val = ""
                for seg in segments:
                    new_val += f"<b>{seg}</b>\n{title_name}\n"
...
if st.button("同步雲端、寄信並下載 PDF 💾", type="primary"):
    ...
    st.success("✅ 同步完成！")
    st.download_button("點此下載 PDF", data=pdf_out, file_name=f"防制危險駕車勤務規劃表.pdf")
