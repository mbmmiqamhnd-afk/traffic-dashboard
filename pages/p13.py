...
def load_data():
    try:
        client = get_client()
        if client is None:
            return None, None, None, "", "Google 授權失敗，請確認 Secrets 中的 private_key 格式"
...
st.subheader("2. 任務編組")
ed_cmd = st.data_editor(df_c, num_rows="dynamic", use_container_width=True).dropna(how="all").fillna("")
