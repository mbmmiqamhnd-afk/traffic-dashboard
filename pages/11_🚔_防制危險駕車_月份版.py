...
# --- 1. 頁面設定 ---
st.set_page_config(page_title="防制危險駕車月份版", layout="wide")
...
def load_data():
    try:
        ...
        df_settings = pd.DataFrame(sh.worksheet("危駕月_設定").get_all_records())
        df_cmd      = pd.DataFrame(sh.worksheet("危駕月_指揮組").get_all_records())
        df_schedule = pd.DataFrame(sh.worksheet("危駕月_勤務表").get_all_records())
...
def generate_html(month, df_cmd, df_schedule):
    ...
    html += f"<h2>{UNIT}{month}執行「防制危險駕車」專案勤務規劃表</h2>"
    ...
    # 執行勤務表
    html += "<div class='section'><b>執行勤務日期、單位及分工</b></div>"
    html += "<table><tr><th width='20%'>日期（22時至翌日6時）</th><th width='20%'>單位</th><th width='60%'>分工</th></tr>"
