...
def process_three_major_daily(files, sh):
    ...
    for f in files:
        ...
        if is_detail:
            ...
            df_det = pd.read_csv(f, skiprows=detail_header_idx, encoding="cp950" if is_csv else None) if is_csv else pd.read_excel(f, skiprows=detail_header_idx)
    ...
    # 1. 產生三項重點違規總表
    p_tot = df_all.pivot_table(index="unit", columns="date_label", values="count", aggfunc="sum", fill_value=0)
    p_tot = p_tot.reindex(columns=date_cols, fill_value=0).astype(int)
    ...
    # 畫面展示
    st.subheader("🚦 取締三項重點違規（闖紅燈、逆向行駛、不停讓行人）每日件數及累計統計表")
    st.caption(f"📅 統計起日：115 年 9 月 1 日 ｜ 涵蓋統計期間：{start_label} 至 {end_label} ｜ 最新統計日：{latest_day}")

    c_m1, c_m2, c_m3, c_m4 = st.columns(4)
    c_m1.metric("🎯 9/1起三項累計總數", f"{df_summary_overview.iloc[0]['三項合計(累計)']} 件")
    c_m2.metric(f"📅 最新單日新增 ({latest_day})", f"{df_summary_overview.iloc[0][f'三項合計({latest_day})']} 件")
    c_m3.metric("闖紅燈 / 逆向累計", f"{df_summary_overview.iloc[0]['闖紅燈(累計)']} / {df_summary_overview.iloc[0]['逆向行駛(累計)']} 件")
    c_m4.metric("不停讓行人累計", f"{df_summary_overview.iloc[0]['不停讓行人(累計)']} 件")
    ...
    if sh:
        try:
            ...
            ensure_ws_capacity(ws, len(df_final_daily) + 10, len(df_final_daily.columns) + 5)
            _ws_clear(ws)

            title_str = f"桃園市政府警察局龍潭分局 取締三項重點違規（闖紅燈、逆向行駛、不停讓行人）每日績效統計表 (115年9月1日至{latest_day})"
            grid_data = (
