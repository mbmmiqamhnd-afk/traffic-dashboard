...
def process_jing_tao(files):
    ...
    靜桃計畫大執法專案統計
    修正重點：
    1. 標題列偵測只需 '通報日期'，時段欄位在讀入後再動態尋找
    2. 時段欄位模糊匹配：支援 22-06 / 22~06 / 2206 / 夜間 / 22:00 等多種格式
    3. 如果找不到標題列，嘗試從 Excel 讀取並自動偵測欄位
    4. 加入詳細的偵錯訊息協助排查
    """
    ...
    for f in files:
        ...
        if text:
            ...
            for idx, line in enumerate(lines[:50]):  # 掃描前50行（原本只掃30行）
                # 放寬條件：只要有「通報日期」就視為標題列
                if '通報日期' in line:
                    target_file = f
                    header_row_idx = idx
                    break
        else:
            # 文字解碼失敗 → 嘗試當作 Excel 處理
            f.seek(0)
            try:
                df_ex = pd.read_excel(f, nrows=50, header=None)
    ...
    if target_file is None:
        ...
        st.warning("📋 以下為上傳檔案的前20行內容，請確認格式是否正確：")
        for f in files:
            f.seek(0)
            raw_bytes = f.read()
            st.write(f"**── 檔案：{f.name} ──**")
            decoded = False
            for enc in ['utf-8-sig', 'utf-8', 'cp950', 'big5']:
                try:
                    text = raw_bytes.decode(enc)
                    ...
                    for i, line in enumerate(lines[:20]):
                        st.code(f"[{i:02d}] {line[:120]}")  # 截斷超長行
                    decoded = True
                    break
                except UnicodeDecodeError:
                    continue
            if not decoded:
                ...
                f.seek(0)
                try:
                    df_ex = pd.read_excel(f, nrows=20, header=None)
                    for i, row in df_ex.iterrows():
                        st.code(f"[{i:02d}] {' | '.join([str(x)[:20] for x in row if pd.notna(x)])}")
                except Exception as e:
                    st.code(f"無法讀取：{e}")
        return
    ...
    target_file.seek(0)
    try:
        if target_encoding == 'excel' or is_excel:
    ...
    if not date_col or not unit_col:
        st.error(f"❌ 缺乏關鍵欄位！目前偵測到的欄位如下：")
        st.write(list(df.columns))
        return

    if not col_22 and not col_06:
        st.warning(f"⚠️ 找不到時段欄位（22-06 / 06-22），將只顯示總計。已偵測到的欄位：{list(df.columns)}")

    # ── 步驟 4：民國年日期解析 ────────────────────────────────────────
    def parse_roc_date(val):
        if pd.isna(val): return pd.NaT
        s = str(val).strip().split(' ')[0]   # 去除時間部分
        # 格式：111/01/01 或 111-01-01
        parts = re.split(r'[/\-]', s)
        if len(parts) == 3:
