...
# 2. 🌟 恢復原廠設定：核心格式化輔助函數 🌟
# ==========================================
def sync_to_specified_sheet(df_final):
    ...
    try:
        ...
        data_body = df_final.values.tolist() 
        data_list = [top_row, bottom_row] + data_body
        
        # 2. 從 A2 開始寫入，保留 A1 總標題格式不變
...
def process_project(files):
    ...
    if not f1 or not f2_list:
        st.error("❌ 強化專案需包含「法條報表(強化)」與「大型車報表(R17)」")
        return
...
def process_accident(files):
    """🚑 交通事故處理邏輯"""
    if len(files) != 4:
        st.error(f"❌ 交通事故需 4 份檔案，目前數量不符 ({len(files)} 份)。")
        return
    ...
    if GCP_CREDS:
        ...
        for ws_idx, df in zip([2, 3], [a1_res, a2_res]):
            ...
            ws.batch_clear(["A2:G20"])  # 完全不碰A1
            
            # 同步標題 (第二列黑紅字)
            reqs = []
            for c_idx, c_name in enumerate(df.columns):
                # 這裡的 row_idx 傳入 1 代表第二列 (Row 2)
