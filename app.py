...
def get_robust_date(df):
    """
    🛠️ 錨點式日期抓取：
    鎖定「統計期間」或「至」關鍵字，避免抓到表格內的舉發件數數字。
    """
    try:
        # 讀取前 5 列文字（通常日期都在這裡）
        header_text = ""
        for i in range(min(5, len(df))):
            header_text += "".join(df.iloc[i].astype(str).tolist())
        
        # 移除干擾字元，保留數字與關鍵字
        clean_text = re.sub(r'[^0-9至\-]', '', header_text)
        
        # 尋找 民國年(1XX) + 月日(4位) + 至 + 民國年(1XX) + 月日(4位)
        # 範例：1150401至1150407
        ...
        # 備援：如果沒有「至」字，改找所有 7 位數
        all_dates = re.findall(r'1\d{6}', clean_text)
        if len(all_dates) >= 2:
            # 確保抓到的是前兩組數字 (標題日期)，而非表格數據
...
def process_major_module(files):
    ...
    if GCP_CREDS:
        if push_to_gsheet(df_result):
            st.success(f"✅ 雲端同步完成！(正確識別期間：{date_wk})")
