...
def process_project(files):
    ...
    if GCP_CREDS:
        ...
        # 🌟 智慧判斷邏輯：找出各項達成率的「後兩名」，若 < 100% 則記錄準備標紅 🌟
        red_cells = []
        for c_idx, cat in enumerate(PROJECT_CATS):
            rate_col_idx = 3 + c_idx * 3  # 在 Google Sheet 中的達成率欄位索引
            valid_rates = []
            
            # 搜集所有單位的達成率 (排除不列入評比的單位)
            ...
            if valid_rates:
                ...
                for row_idx, rate_val in bottom_2:
                    # 💡 核心判斷：未達 100% 才會被送上紅字名單
                    if rate_val < 100.0:
                        red_cells.append((3 + row_idx, rate_col_idx)) # +3 是因為資料從 Row 4 開始
