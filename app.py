...
def get_robust_date(df):
    """
    🎯 座標鎖定法：
    直接讀取第三列 (Index 2) 的文字內容，精確擷取入案日的日期區間。
    """
    try:
        # 取得第三列的內容 (通常在第一欄或合併單元格中)
        # 我們將第三列的所有內容串接起來搜尋，以防格式偏移
        target_row_text = "".join(df.iloc[2].astype(str).tolist())
        
        # 移除空格與特殊符號，保留數字與「至」
        clean_text = re.sub(r'[^0-9至]', '', target_row_text)
        
        # 搜尋模式：1XX(年)MMDD(月日) + 至 + 1XX(年)MMDD(月日)
        # 範例：1150401至1150407 -> 擷取 0401 與 0407
        pattern = r'1\d{2}(\d{4})至1\d{2}(\d{4})'
        match = re.search(pattern, clean_text)
        
        if match:
            return f"{match.group(1)}-{match.group(2)}"
            
        # 備援：若格式不標準，找前兩個符合 1XXXXXX 的數字
        all_dates = re.findall(r'1\d{6}', clean_text)
        if len(all_dates) >= 2:
            return f"{all_dates[0][-4:]}-{all_dates[1][-4:]}"
...
def process_major_module(files):
    ...
    if GCP_CREDS:
        if push_to_gsheet(df_result):
            st.success(f"✅ 雲端同步成功！(期間識別：{date_wk})")
...
uploaded_files = st.file_uploader("📂 上傳重大違規報表", type=["xlsx", "xls"], accept_multiple_files=True)
