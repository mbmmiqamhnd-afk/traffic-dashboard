...
def process_project(files):
    ...
    if GCP_CREDS:
        ...
        # 🌟 智慧判斷邏輯：找出各項達成率未達 100% 的「後兩名」標紅 🌟
        red_cells = []
        for c_idx, cat in enumerate(PROJECT_CATS):
            rate_col_idx = 3 + c_idx * 3  # 達成率在 Google Sheet 中的欄位索引
            valid_rates = []
            ...
            if valid_rates:
                # 排序並找出最差的兩個 (若平手 Python 也只會嚴格抓兩個)
...
if app_mode == "🏠 智慧批次處理中心":
    ...
    st.markdown("您可以選擇 **拖入檔案**，或直接點擊 **自動載入** 讀取下載機器人剛抓好的報表。")
    ...
    if st.button("🔄 自動載入『今日待上傳報表』資料夾", use_container_width=True, type="primary"):
        ...
        if folder_path:
            ...
            # 抓取資料夾內所有的 Excel 與 CSV 檔案
            local_files = glob.glob(os.path.join(folder_path, "*.*"))
            auto_uploads = [LocalMockFile(f) for f in local_files if f.endswith(('.xlsx', '.xls', '.csv'))]
