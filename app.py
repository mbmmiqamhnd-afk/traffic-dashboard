...
def get_robust_date(df):
    """強健式日期擷取：掃描前10列文字，擷取第一組與最後一組民國日期"""
    try:
        text_block = df.head(10).astype(str).to_string()
        # 匹配民國格式 115/03/01 或 115年03月01日
        date_ptn = r'(1\d{2})[/年\-]([0-1]?\d)[/月\-]([0-3]?\d)'
        matches = re.findall(date_ptn, text_block)
        
        if len(matches) >= 2:
            start_label = f"{int(matches[0][1]):02d}{int(matches[0][2]):02d}"
            end_label = f"{int(matches[-1][1]):02d}{int(matches[-1][2]):02d}"
...
def process_major_module(files):
    """重大違規處理邏輯"""
    if len(files) < 2:
        st.error("❌ 錯誤：必須同時上傳『本期報表』與『(1)累計報表』。")
        return

    # --- 1. 檔案分類 (避開數據相同問題) ---
    # 規則：檔名含 (1) 為累計檔；若皆無，則檔案體積較大者為累計檔
    f_year = next((f for f in files if "(1)" in f.name), None)
    if f_year:
        f_wk = next((f for f in files if f != f_year), None)
    else:
        sorted_files = sorted(files, key=lambda x: x.size)
        f_wk, f_year = sorted_files[0], sorted_files[1]

    st.write(f"📂 系統識別：**本期檔** ({f_wk.name}) | **累計檔** ({f_year.name})")
    ...
    for u in MAJOR_UNIT_ORDER:
        ...
        table_rows.append([u, w_data['stop'], w_data['cit'], y_total - w_data['stop'] - w_data['cit'] if u == '不可靠' else y_data['stop'], y_data['cit'], l_data['stop'], l_data['cit'], diff if u != '警備隊' else "—", tgt, rate if u != '警備隊' else "—"])
    ...
    if GCP_CREDS:
        if push_to_gsheet(df_result):
            st.success(f"✅ 雲端同步成功！(本期期間：{date_wk})")

def push_to_gsheet(df):
    """推送到 Google Sheets，保留 A1"""
    try:
...
uploaded_files = st.file_uploader("📂 請同時拖入本週報表與(1)累計報表", type=["xlsx", "xls"], accept_multiple_files=True)

if uploaded_files:
    if st.button("🚀 執行重大違規自動判別"):
        with st.spinner("正在解析報表結構..."):
            # 僅篩選重大違規相關檔案
            target_files = [f for f in uploaded_files if "重大" in f.name or "重點" in f.name]
