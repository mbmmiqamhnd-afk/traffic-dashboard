...
def show_sidebar():
    with st.sidebar:
        ...
        # 🧪 【新增】全自動簡報生成測試功能（免試算表、免母本）
        st.page_link("pages/p32.py", label="專案簡報極速直出 (免母本測試)", icon="📽️")
        
        st.page_link("pages/p27.py", label="舉發績效結算系統", icon="👮")
        # ✅ 危險駕車獎勵統計
        st.page_link("pages/p28.py", label="危險駕車取締獎勵統計", icon="🚨") 
        # ✅ 無照駕駛車輛移置敘獎統計
        st.page_link("pages/p29.py", label="無照駕駛車輛移置敘獎統計", icon="🚗")
        # ✅ 偽變造車牌專案敘獎統計
        st.page_link("pages/p30.py", label="偽變造車牌專案敘獎統計", icon="🔎")
        # ✅ 新增毒駕專案取締敘獎統計（支援第35條、第73條細項過濾）
        st.page_link("pages/p31.py", label="毒駕專案取締敘獎統計", icon="🧪")
...
def main():
    ...
    ✅ **最新系統更新：**
    * 新增 **[專案簡報極速直出 (免母本測試)] (p32.py)**：跳過 Google Sheets 中繼站，支援從零全新建立 Google Slides 簡報，直接繪製原生表格與警政深藍色封面。
    * 已於「數據與分析」專區新增 **[毒駕專案取締敘獎統計] (p31.py)**，精準區分第35條與第73條之毒駕本體、毒駕累犯、毒駕拒測及慢車毒駕，自動產製符合市警局「加強攔查取締施用毒品後駕車專案工作計畫」（桃警交字第1150120088號函）之出力人員敘獎名冊與建議表。
    * 已於「數據與分析」專區新增 **[偽變造車牌專案敘獎統計]**，可自動依據簡式車種與違規事實精算嘉獎次數，並內建滿9次加倍與他車牌照案件折算機制。
    * 已於「數據與分析」專區新增 **[無照駕駛車輛移置敘獎統計]**，可自動依新年度季別、年齡及法條條件結算，並處理未達標保留件數與額度上限。
    * 已於「數據與分析」專區新增 **[危險駕車取締獎勵統計]**，可自動依據法條與違規事實，精準核算 2件/4件 嘉獎群組與總大功數。
